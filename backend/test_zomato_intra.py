...
async def test_intraday_zomato():
    ...
    try:
        df_15m = await market_service.get_ohlc(symbol, period="5d", interval="15m", fast_fail=True)
    ...
    print(f"\nFetching 5m OHLC (2d)...")
    try:
        df_5m = await market_service.get_ohlc(symbol, period="2d", interval="5m", fast_fail=True)
