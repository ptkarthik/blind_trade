...
def map_sector(industry):
    ind = industry.upper()
    if "BANK" in ind or "FINANCIAL" in ind: return "Banking"
    if "IT" in ind or "SOFTWARE" in ind or "TECHNOL" in ind: return "IT"
    if "AUTO" in ind: return "Auto"
    if "PHARMA" in ind or "HEALTH" in ind: return "Pharma"
    if "ENERGY" in ind or "POWER" in ind or "OIL" in ind: return "Energy"
    if "CONSUMER" in ind or "GOODS" in ind or "FMCG" in ind: return "FMCG"
    if "METAL" in ind or "STEEL" in ind: return "Metal"
    if "CONSTRUCTION" in ind or "INFRA" in ind: return "Infrastructure"
    if "TEXTILE" in ind: return "Textiles"
    if "CHEMICAL" in ind: return "Chemicals"
    if "SERVICES" in ind: return "Services"
    return "Others"

for chunk in chunks:
    lines = chunk.strip().split('\n')
    if lines[0].startswith("Company Name"): lines = lines[1:]
    for line in lines:
        parts = line.split(',')
        if len(parts) >= 3:
            name = parts[0]
...
# Final addition of any missing key symbols
extras = [
    {"symbol": "IDEA.NS", "name": "Vodafone Idea", "sector": "Telecom"},
    {"symbol": "SUZLON.NS", "name": "Suzlon Energy", "sector": "Energy"},
    {"symbol": "ZOMATO.NS", "name": "Zomato", "sector": "Services"},
]
for ex in extras:
    if ex["symbol"] not in seen_symbols:
        master_list.append(ex)
        seen_symbols.add(ex["symbol"])

output_path = "backend/app/data/nifty500.json"
os.makedirs(os.path.dirname(output_path), exist_ok=True)
with open(output_path, "w") as f:
    json.dump(master_list, f, indent=4)

print(f"Successfully generated {len(master_list)} symbols in {output_path}")
print(f"Suzlon in list? {'SUZLON.NS' in seen_symbols}")
print(f"Idea in list? {'IDEA.NS' in seen_symbols}")
